...
public class CadastroEnfermeiroController implements Initializable {
    ...
    private void botaoCadastrar(ActionEvent event) throws FileNotFoundException {
        ...
        for (int i = 0; i < Hospital.listaEnfermeiro.size(); i++) {
            if (login.getText().equals(Hospital.listaEnfermeiro.get(i).login)) {
                condCadastro = false;
            }
        }
        ...
        if (condCadastro) {
            ...
            Enfermeiro enfermeiro = new Enfermeiro(nome.getText(), endereco.getText(), cpf.getText(), new Data(diaint, mesint, anoint), telefone.getText(), email.getText(), login.getText(), senha.getText(), perio);
            ...
            dialogoInfo.setHeaderText("O enfermeiro " + nome.getText() + " foi cadastrado com sucesso!");
